  ...
  const analyzeImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        // For demo, we'll analyze the image with Gemini Vision
        const imageAnalysisPrompt = `Analyze this image for potential misinformation, fake news, or misleading content. Look for:
1. Manipulated or doctored images
2. Misleading captions or text overlays
3. Out-of-context usage
4. Propaganda elements
5. Suspicious visual elements

Provide a detailed fake news analysis with a score from 0-100.`;
        
        const analysis = await analyzeImageWithGemini(base64, imageAnalysisPrompt);
        const fakeScore = extractFakeScore(analysis);
        const newResult = {
          id: Date.now(),
          type: 'Image Analysis',
          content: `Analyzed uploaded image: ${imageUri.split('/').pop()}`,
          analysis: analysis,
          fakeScore: fakeScore,
          timestamp: new Date().toLocaleString()
        };
  ...
  const analyzeDocument = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        setIsAnalyzing(true);
        setAnimationType('document');
        setShowUploadAnimation(true);
        const document = result.assets[0];
        
        // For demo purposes, we'll simulate document text extraction
        // In a real app, you'd use libraries like react-native-pdf or similar
        const simulatedText = `Document content from ${document.name}: This document contains various claims that need fact-checking. The content discusses recent political events and makes several unverified statements about government policies.`;
        
        const analysis = await analyzeWithGemini(simulatedText, 'document');
        const fakeScore = extractFakeScore(analysis);
        const newResult = {
          id: Date.now(),
          type: 'Document Analysis',
          content: `Analyzed document: ${document.name} (${(document.size / 1024).toFixed(1)} KB)`,
          analysis: analysis,
          fakeScore: fakeScore,
          timestamp: new Date().toLocaleString()
        };
  ...
  const analyzeVideo = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        setIsAnalyzing(true);
        setAnimationType('video');
        setShowUploadAnimation(true);
        const videoUri = result.assets[0].uri;
        
        // For demo purposes, simulate video transcript extraction
        // In a real app, you'd use speech-to-text services
        const simulatedTranscript = `Video transcript from uploaded file: The speaker discusses various controversial topics and makes several claims about recent events. The video contains emotional language and presents information without citing credible sources.`;
        
        const analysis = await analyzeWithGemini(simulatedTranscript, 'video transcript');
        const fakeScore = extractFakeScore(analysis);
        const newResult = {
          id: Date.now(),
          type: 'Video Analysis',
          content: `Analyzed video: ${videoUri.split('/').pop()} (${(result.assets[0].duration / 1000).toFixed(1)}s)`,
          analysis: analysis,
          fakeScore: fakeScore,
          timestamp: new Date().toLocaleString()
        };
  ...
  const analyzeAudio = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        setIsAnalyzing(true);
        setAnimationType('audio');
        setShowUploadAnimation(true);
        const audio = result.assets[0];
        
        // For demo purposes, simulate speech-to-text conversion
        // In a real app, you'd use services like Google Speech-to-Text or Azure
        const simulatedTranscript = `Audio transcript from ${audio.name}: The speaker makes various claims about current events and expresses strong opinions about political matters. The audio contains emotional appeals and presents information that requires fact-checking.`;
        
        const analysis = await analyzeWithGemini(simulatedTranscript, 'audio transcript');
        const fakeScore = extractFakeScore(analysis);
        const newResult = {
          id: Date.now(),
          type: 'Audio Analysis',
          content: `Analyzed audio: ${audio.name} (${(audio.size / 1024).toFixed(1)} KB)`,
          analysis: analysis,
          fakeScore: fakeScore,
          timestamp: new Date().toLocaleString()
        };
      ...
      setIsAnalyzing(false);
      Alert.alert('Error', 'Failed to analyze audio: ' + error.message);
    }
  };
      ...
      <View style={styles.mediaSection}>
        ...
        <View style={styles.mediaButtons}>
          <TouchableOpacity 
            ...
            disabled={isAnalyzing}
          >
            <Text style={styles.mediaButtonText}>📷 Image OCR</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={styles.mediaButton} 
